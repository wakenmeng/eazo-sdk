...
UI code should use the React helpers so checkout, login, and entitlement state
stay on the SDK-owned lifecycle:

```tsx
import { EazoEntitlementGate, EazoPaymentButton } from "@eazo/sdk/payments/react";

<EazoPaymentButton productKey="premium" unlockedChildren="Premium active">
  Upgrade
</EazoPaymentButton>
...
For custom UI, call `startEazoCheckout(productKey)` from `@eazo/sdk/payments`
and read unlock state with `useEazoEntitlement(productKey)` from
`@eazo/sdk/payments/react`. Checkout requires Eazo Auth first; if the current
app user is not signed in, SDK helpers open the login flow before creating
checkout.
