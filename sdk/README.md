...
UI code should call the client helper:

```tsx
import { startEazoCheckout } from "@eazo/sdk/payments";

<button onClick={() => startEazoCheckout("premium")}>Upgrade</button>
```
...
Do not add Stripe SDKs, Stripe webhooks, `STRIPE_SECRET_KEY`, or direct `/api/open/payments/*` request bodies to generated apps. Checkout uses same-window navigation and confirms payment by polling the Eazo ledger after Stripe returns with `payment_id`.
