...
import type { EazoAuthServerConfig, UserInfo } from "./types";
import type { SessionToken } from "./EazoAuthClient";

/**
 * Server-side auth verifier.
 * Decrypts an encrypted session token (produced by Eazo Mobile bridge or
 * the /api/open/app-session-token exchange) using EAZO_PRIVATE_KEY.
 * Both Mobile and Web produce the same SessionToken shape, so verification
 * is a single code path with no environment detection.
 */
export class EazoAuthServer {
  private readonly privateKey: string;

  constructor(config: EazoAuthServerConfig) {
    if (!config.privateKey) throw new Error("@eazo/auth: privateKey is required");
